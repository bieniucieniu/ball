...
fn TagedSap(T: type) type {
    return struct {
        ...
        fn init(alloc: std.mem.Allocator, init_size: usize) !@This() {
            return .{
                .alloc = alloc,
        ...
        fn copyQuads(s: *@This(), quads: []const Q) !void {
        ...
        fn setQuadsAsSlice(s: *@This(), size: usize) []Q {
            if (size != s.quads.len) {
                s.alloc.free(s.quads);
        ...
        fn reset(s: *@This(), capacity: usize) !void {
        ...
        fn runWith(s: *@This(), quads: []const Q) ![]const Q {
            try s.copyQuads(quads);
            std.mem.sort(Q, s.quads, {}, Q.minAsc);
            return s.run();
        }
        fn run(s: *@This()) ![]const Q {
            try s.reset(s.quads);
            ...
            for (s.quads) |q| {
                ...
                inner: while (j < s.active_list.items.len) : (j += 1) {
                    ...
                    if (q.min > a.max) {
                        ...
                        s.all_pairs.append(s.alloc, .{ q.tag, a.tag });
                    }
                }
                std.debug.print("added to activeList:\n\t{}\n", .{q});
                s.active_list.append(s.alloc, q);
            }
            return s.quads;
        }
    };
